...
OUTMP4=sys.argv[1] if len(sys.argv)>1 else "/tmp/yt/dreamlayer_010_voiced.mp4"
SECTS={}
...
for i in range(len(gfr)):
    emit(a_frame(i))
for k in range(72): emit(a_frame(len(gfr)-1))
...
for k in range(72): emit(a_frame(len(gfr)-1,shelf=1.0))
print("A",NF)
...
for k in range(42): emit(b_frame())
print("B",NF)
...
EXTRA={"home":125,"day":117,"mind":209,"reach":49,"privacy":128,"plugins":135,"caps":104,"learn":82,"advanced":33}
import math
bounds=[]; idx=40   # settle frames at start of tour
for t in ORDER:
    n=20+24+math.ceil(SPANS[t]/16)+26
    bounds.append((t,idx,idx+n)); idx+=n
extra={t:EXTRA[t] for t,s,e in bounds}
...
for i,fp in enumerate(tfr):
    ...
    emit(im)
    for t,s,e in bounds:
        if i==s: timeline.append((t,NF-1)); SECTS[t]=NF-1
    if i in hold_at:
        for _ in range(hold_at[i]): emit(im)
SECTS['outro']=NF-70   # cursor pull-away + end card carry the outro line
print("D",NF)
print("TIMECODES (s):")
...
import json as _j; _j.dump({k:v/FPS for k,v in SECTS.items()},open("/tmp/yt/sections.json","w"),indent=1)
print("total",NF,"->",OUTMP4)
print("SECTIONS:",{k:round(v/FPS,2) for k,v in SECTS.items()})
