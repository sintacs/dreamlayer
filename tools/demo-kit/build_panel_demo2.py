"""Frame the driven walkthrough (viewport screenshots + caps.json) into the Mac
app window with per-frame captions. Output frames -> <out>/frames."""
...
SEQ=Path("/tmp/panel_shots2/seq")
caps=json.loads(Path("/tmp/panel_shots2/caps.json").read_text())
...
ImageDraw.Draw(BASE).text((CW//2,132),"Plug in any API as your brain",font=D.font(34),fill=INK,anchor="mm")
WIN_X=(CW-sh0.width)//2; WIN_Y=196
...
for i,fp in enumerate(frames):
    ...
    if t: d.text((CW//2,CH-120),t,font=D.font(30),fill=BRAND,anchor="mm")
    if s: d.text((CW//2,CH-80),s,font=D.font(19,False),fill=DIM,anchor="mm")
