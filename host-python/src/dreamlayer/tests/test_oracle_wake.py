"""test_oracle_wake.py — "Hey Oracle", multimodal activation, and the
...
def test_detect_wake_hey_oracle_and_remainder():
    assert detect_wake("Hey Oracle, what's my day?") == (True, "what's my day?")
    assert detect_wake("oracle") == (True, "")
    assert detect_wake("okay oracle brief me") == (True, "brief me")
    # not addressed / a word that merely contains the name
    assert detect_wake("the oracle of Delphi") == (False, "the oracle of Delphi")
    assert detect_wake("what did Marcus need") == (False, "what did Marcus need")
    assert ASSISTANT_NAME == "Oracle"
    assert strip_wake("Hey Oracle, brief me") == "brief me"      # back-compat helper
...
def test_not_addressed_when_asleep_is_idle():
    ...
    assert not orc.oracle_listening()


def test_session_expires():
    orc = Orchestrator(FakeBridge())
    orc.hear("Hey Oracle", now=1000.0)
    assert orc.oracle_listening(now=1005.0)
    assert not orc.oracle_listening(now=1000.0 + orc.oracle_session_s + 1)
...
def test_disabled_wake_source_does_nothing():
    ...
    assert orc.activate("gaze") is None and not orc.oracle_listening()


def test_voice_wake_can_be_disabled():
    orc = Orchestrator(FakeBridge())
    orc.set_wake_source("voice", False)
    assert orc.hear("Hey Oracle, brief me") == {"intent": "idle"}
