...
from .renderer import LieLensRenderer
from .narrative_store import NarrativeStore
from .schema import (
    AUFrame, ProsodyFrame, LinguisticFrame, LieLensResult,
)
...
class LieLens:
    ...
    def __init__(
        ...
        self._renderer = LieLensRenderer()
    ...
    def tick(self) -> Optional[LieLensResult]:
        """Run fusion + render. Return LieLensResult if ready to emit."""
        if not self._privacy.allow_capture():
            return None
        now = time.monotonic()
