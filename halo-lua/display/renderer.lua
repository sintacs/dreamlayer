...
--- Every render path: frame.display.clear(0) → draw commands → frame.display.show()
---
--- New primitives (quadratic_bezier, elliptical_arc, etc.) are implemented
...
--- Convert 0xRRGGBB integer to the value frame.display.* expects.
--- frame API takes the integer directly.
...
local function bezier(p0x, p0y, p1x, p1y, p2x, p2y, color, steps)
  ...
  for i = 1, steps do
    ...
    -- Dash: skip every other segment group (frozen offset simulates dash)
...
local function radial_rays(cx, cy, count, lengths, color, bloom_r)
  bloom_r = bloom_r or 2
  local step = (2 * math.pi) / count
  for i = 0, count - 1 do
    local angle  = step * i - math.pi / 2
    local length = lengths[(i % #lengths) + 1] or 40
    local x2 = cx + length * math.cos(angle)
    local y2 = cy + length * math.sin(angle)
    frame.display.line(floor(cx), floor(cy), floor(x2), floor(y2), color)
    -- tip bloom
...
local function check_glyph(cx, cy, size, color, progressive)
  ...
  -- progressive=0.6 means draw first segment + 20% of second
...
local function draw_query_listening()
  ...
  -- Sine-envelope waveform: 32 bars approximated as vertical lines
  local bar_count = 32
  local bar_w     = 2
  local gap       = 1
...
local function draw_object_recall(card)
  ...
  local jx, jy = 174, 112   -- approx apex at t=0.5
  local jd = 4
  ...
  -- Place hero text (at curve endpoint, offset-left)
...
local function draw_commitment_recall(card)
  ...
  -- Chain links: 3 rounded rects via rect + polyline outlines
  ...
  for li, ly in ipairs(link_ys) do
    ...
    -- Outline the rect as 4-segment polyline
    polyline({
      {lx, ly}, {lx+link_w, ly},
...
local function draw_proactive_memory(card)
  ...
  -- Radial ray field (5 rays)
  radial_rays(CX, CY - 10, 5, {38, 52, 44, 30, 46}, P.memory_trace, 2)
...
--- Redraws current card for simple animation (spinner, etc.)
function renderer.tick()
  _tick_count = _tick_count + 1
  -- Redraw at ~10fps to animate loading/ready cards
  if _current_card and _tick_count % 1 == 0 then
...
function renderer.push(layer, fn) fn() end
