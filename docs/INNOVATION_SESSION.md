...
**Why only DreamLayer:** You can register a *perceptor* — a competitor to the platform's own vision tier — as a plugin. That is structurally unthinkable on any closed device. Bonus: submissions become the candidate pool for real Ethos-U55 Vela models later (the winner gets quantized).
...
**How it works:** The pieces are merged and tested: `plugins/wasm_host.py` (WASM plugin host, wasmtime/WASI — the repo's recent commits wire this as "the strongest untrusted-plugin jail"), `plugins/isolation.py` (subprocess proxy under the glance deadline), `plugins/os_sandbox.py` (documented zero-ambient-authority posture, "not yet wired" — wire it), and `plugins/package.py` entitlements. The contract: an untrusted perceptor receives pixels and returns labels; it cannot open sockets, files, or clocks. Combine with 1.4's leaderboard: unreviewed community perceptors become *installable* because the jail, not a reviewer, is the guarantee.
...
- **Inputs:** ESP32/RPi sensors POSTing to the Brain (`ble:<n>` event injection, Category 1.6), figment event grammar.
- **Outputs:** event-driven scene transitions (MAIL / DOOR / KETTLE), phone `notice` haptic, telemetry row per event.
- **Dataflow:** sensor → LAN POST → Brain route → `figment_event` envelope → active figment's `ble:<n>` exit fires. The figment defines the *entire* policy: which events matter, what shows, for how long — statically proven, so a chatty sensor cannot flood you (emit budgets bound the reverse path; scene min-durations bound the display path).
- **Magic moment:** The first time a $6 reed switch changes what your glasses display, and you check the wire format: a 5-integer JSON envelope.
- **Composes with:** Confluence (a bonded partner's Wake events can cross: "his workshop door opened" as an ambient mark on your Horizon ring). *Only an embedded developer thinks of the display as an interrupt target with provable rate limits.*
...
4. **The "LLM parser" is regex in a trenchcoat.** `reality_compiler/intent_parser_llm.py: _llm_parse()` concatenates the model hint and re-runs the deterministic matcher; outlines/instructor are imported and unused. Either wire true constrained decoding (outlines' JSON-schema mode over the local Ollama model) or delete the file and own the beautiful truth — the deterministic grammar *is* the safety story (5.4). What's there now is the worst option: an aspiration that reads like a capability.
